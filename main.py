...
latency_models = {
    ...
    ('Top', 'Target'):   {'type': 'highway', 'f': lambda x: 45.0,      'label_base': '45'},
    ('Start', 'Bottom'): {'type': 'highway', 'f': lambda x: 45.0,      'label_base': '45'},
...
print("\n[Part A: Saving all visualization steps as individual PNGs]...")
total_saved_pngs = 0
for idx, data in enumerate(frames_data):
    render_current_state(main_ax, data)
    
    filename = f"{fig_dir}/step_{idx:03d}.png"
    plt.savefig(filename, dpi=200, bbox_inches='tight')
    total_saved_pngs += 1
    if total_saved_pngs % 10 == 0 or total_saved_pngs == len(frames_data):
        print(f" Saved: {total_saved_pngs} PNG images...")

print(f"Successfully saved {total_saved_pngs} PNG images to the '{fig_dir}/' directory.")
...
gif_output_path = 'figs/braess_paradox_complete_simulation.gif'
